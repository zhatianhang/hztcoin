...
#if defined(NDEBUG)
# error "Hzcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x29788fa1bc2c5b60197cd18cfc0c5c2ca7919722ea211f4c9b0738a7a3fb20e4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Hzcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Hzcoin Signed Message:\n";
...
{
    // Hzcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Hzcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Hzcoin: 3.5 days
static const int64 nTargetSpacing = 1 * 30; // Hzcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Hzcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Hzcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Hzcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x29788fa1bc2c5b60197cd18cfc0c5c2ca7919722ea211f4c9b0738a7a3fb20e4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "ztcoin is made by zhatian";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("123454710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1565362165;
        ...
        assert(block.hashMerkleRoot == uint256("0xd50b5c9ea56ea37b0a4d75e0270148aa396dd3022c235e9e9232535a2e1123e9"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Hzcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HzcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HzcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HzcoinMiner(CWallet *pwallet)
{
    printf("HzcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hzcoin-miner");
    ...
    try { loop {
        ...
        printf("Running HzcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HzcoinMiner, pwallet));
}
